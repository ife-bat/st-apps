...
uploaded_schedule = widget.file_uploader(
    "Upload schedule file (*.sdu, *.sdx)", type=["sdu", "sdx"]
)
...
def updateplot():
    ...
    if st.session_state["new_tab"]:
        plot = st.session_state["tester"].make_overview_bokeh(
            fig_width=st.session_state["fig_width"] * 2,
            fig_height=st.session_state["fig_height"] * 2,
            line_width=1.5,
            line_alpha=0.9,
            show_plot=True,
            normalize=True,
            vertical_stack=(st.session_state["layout"].lower() == "vertical"),
        )
        bplt.show(plot)
    else:
        plot = st.session_state["tester"].make_overview_bokeh(
...
if "tester" in st.session_state:
    with widget.form("fig_params"):
        ...
        subcol1.selectbox("Figure layout:", ["Grid", "Vertical"], index=0, key="layout")
        subcol2.number_input(
            "Plot width:",
            min_value=100,
            max_value=5000,
